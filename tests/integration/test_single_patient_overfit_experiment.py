...
_VOLUME_COUNT: Final[int] = 2
_BATCH_COUNT: Final[int] = 2


@pytest.mark.acdc
@pytest.mark.integration
def test_single_patient_overfit_experiment_runs_on_real_ed_and_es_volumes(
    tmp_path: Path,
) -> None:
    """Run the diagnostic experiment against one real ACDC training patient."""
    ...
    test_config = replace(
        ...
        base_channels=2,
        device="cpu",
        checkpoint_path=tmp_path / "single_patient_overfit.pt",
    )

    experiment = SinglePatientOverfitExperiment(
        app_config=app_config,
        overfit_config=test_config,
    )

    assert experiment.patient_id == "patient001"
    assert experiment.device == torch.device("cpu")
    assert experiment.dataset_volume_count == _VOLUME_COUNT
    ...
    assert not (tmp_path / ".single_patient_overfit.pt.tmp").exists()


def _assert_history_matches_contract(
    history: SegmentationTrainingHistory,
) -> None:
    """Verify completed epochs, metrics, durations, and checkpoint path."""
    ...
    for record in history.epoch_records:
        assert record.training_result.volume_count == _VOLUME_COUNT
        assert record.validation_result.volume_count == _VOLUME_COUNT
        assert record.training_result.batch_count == _BATCH_COUNT
        assert record.validation_result.batch_count == _BATCH_COUNT
        ...
        assert record.training_result.dice_result.included_class_indices == (1, 2, 3)
        assert record.validation_result.dice_result.included_class_indices == (1, 2, 3)
